...
if(isset($_GET['direct_run_competitor_id'])) runCompetitor((int)$_GET['direct_run_competitor_id'], false);
...
function runCompetitor($id, $return = false)
{
	...
	*/
	...
	$rowsearch = array();
	$columnsearch = array();
	$query = 'SELECT `header`,`num`,`type`,searchdata FROM rpt_competitor_headers WHERE report=' . $id;
	...
	if(!$return)
	{
		$query = 'INSERT INTO progress SET created=NOW(),user=' . $db->user->id . ',what="competitor",max='
	...
	//get searchdata
	$searchdata = array();
	$query = 'SELECT `row`,`column`,`searchdata` FROM rpt_competitor_cells WHERE report=' . $id;
	...
	$hasPhase = array();
	/* In Heatmap this generates the phase colors and legend text
	*/
	...
	foreach($searchdata as $row => $rowData)
	{
		foreach($rowData as $column => $cell)
		{
			...
			/*
			- process backbone agent in heatmap
			-			
			-
			*/
			...
			*/
			...
			if($rescount)
			{
				...
				$results[$row][$column]->phase = $key;
				//add to search data, requirement 
				$params[] = $hasPhase[$results[$row][$column]->phase];
				//get number of records
				$all_ids = search($params,array(),NULL,$time_machine);
				if($all_ids === false) return softDie("Search (count) failed.");
				$all_ids = array_keys($all_ids);
				$rescount = count($all_ids);
				$results[$row][$column]->num = $rescount;
			}
	...
	foreach($results as $row => $rowData)
	{
		foreach($rowData as $col => $result)
		{
			...
			/*
				these lines would do color in a heatmap
			*/
			if($result->num)
			{
				$clink = urlPath() . 'intermediary.php?' . $result->{'link'};
				$clink = addYourls($clink,$result->reportname);
				$sheet->SetCellValue($cell, $phases[$result->phase]);
	...
	/*
		these lines would print the legend in a heatmap
		-
		-
		-
		-
