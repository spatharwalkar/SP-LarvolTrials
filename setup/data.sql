...
VALUES(1, 'New trial', 'New Trial', 6, NULL, "select larvol_id, cast(coalesce( firstreceived_date,current_date())as date) as added from data_trials dt where adddate(firstreceived_date,%d)>= current_date() and (dt.phase = \"N/A\" or dt.phase=\"0\")",'B40E52DE-A4D6-4C85-8B48-F662A69F4503',NULL),
	...
	(13, 'Trial completion date', 'Trial status', 3, 'Trial completion date: [date_format(completion_date_prev,\'%b %Y\')] ->[date_format(completion_date,\'%b %Y\')]', "select larvol_id, cast(coalesce( completion_date_lastchanged,current_date())as date) as added from data_history join data_trials dt using (larvol_id) where adddate(completion_date_lastchanged,%d)>= current_date() and adddate(completion_date_prev,90)<=dt.completion_date ",'73DEB650-E6F3-40B0-A056-BF1F93D2AE57',NULL),
