...
VALUES(1, 'New trial', 'New Trial', 6, NULL, "select larvol_id, cast(coalesce( firstreceived_date,current_date())as date) as added from data_trials dt where adddate(firstreceived_date,%d)>= current_date() and (dt.phase = \"N/A\" or dt.phase=\"0\")",'B40E52DE-A4D6-4C85-8B48-F662A69F4503',NULL),
	...
	(21, 'Phase Classification', 'Clinical Trial status', 2, 'Phase classification: P[phase_prev] -> P[phase]', "select larvol_id, cast(coalesce( phase_lastchanged,current_date())as date) as added from data_history join data_trials dt using (larvol_id) where adddate(phase_lastchanged,%d)>= current_date() and phase_prev!=dt.phase",'51BCAC2B-6EB7-42F8-850B-C03E9DA0A287',NULL),
