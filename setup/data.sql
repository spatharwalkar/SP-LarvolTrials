...
INSERT INTO `redtags`(`id`, `name`, `type`, `rUIS`, `formula`, `statement`, `LI_id`)
VALUES(1, 'New trial', 'New Trial', 6, NULL, "select larvol_id, cast(coalesce( firstreceived_date,current_date())as date) as added from data_history join data_trials dt using (larvol_id)  where adddate(firstreceived_date,%d)>= current_date() and overall_status_prev is null and (dt.phase = 'N/A' or dt.phase=0)",'B40E52DE-A4D6-4C85-8B48-F662A69F4503'),
	(2, 'New P4 trial', 'Clinical Trial status', 8, NULL, "select larvol_id, cast(coalesce( firstreceived_date,current_date())as date) as added from data_history join data_trials dt using (larvol_id) where adddate(firstreceived_date,%d)>= current_date() and overall_status_prev is null and dt.phase = '4'",'43EBF0ED-D640-47D1-BA60-0D73B72282F2'),
	(3, 'New P3 trial', 'Clinical Trial status', 10, NULL, "select larvol_id, cast(coalesce( firstreceived_date,current_date())as date) as added from data_history join data_trials dt using (larvol_id) where adddate(firstreceived_date,%d)>= current_date() and overall_status_prev is null and dt.phase = '3'",'3946F04E-A3BB-4DA3-9850-ADC5AAFC161D'),
	(4, 'New P2/P3 trial', 'Clinical Trial status', 10, NULL, "select larvol_id, cast(coalesce( firstreceived_date,current_date())as date) as added from data_history join data_trials dt using (larvol_id) where adddate(firstreceived_date,%d)>= current_date() and overall_status_prev is null and dt.phase = '2/3'",'0BE6A68E-0DE1-413D-83B7-FD8998BED7E6'),
	...
	(22, 'Phase Shift', 'Clinical Trial status', 10, '[phase_prev] -> [phase]', "select larvol_id, cast(coalesce( phase_lastchanged,current_date())as date) as added from data_history join data_trials dt using (larvol_id) where adddate(phase_lastchanged,%d)>= current_date() and phase_prev != dt.phase",'7A4EB458-CD4B-45F0-84A9-D887893215A0'),
	(23,'Clinical Trial,Phase I','Other',4,'Phase 1 Data','SKIP',"select pm_id from pubmed_abstracts where publication_type = 'Clinical Trial,Phase I'"),
	(24,'Clinical Trial,Phase II','Other',7,'Phase 2 Data','SKIP',"select pm_id from pubmed_abstracts where publication_type = 'Clinical Trial,Phase II'"),
	(25,'Clinical Trial,Phase III','Other',8,'Phase 3 Data','SKIP',"select pm_id from pubmed_abstracts where publication_type = 'Clinical Trial,Phase III'"),
	(26,'Clinical Trial,Phase IV','Other',7,'Phase 4 Data','SKIP',"select pm_id from pubmed_abstracts where publication_type = 'Clinical Trial,Phase IV'"),
	(27,'Review','Other',5,'Review','SKIP',"select pm_id from pubmed_abstracts where publication_type = 'Review'");
