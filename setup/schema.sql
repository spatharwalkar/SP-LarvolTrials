...
CREATE TABLE IF NOT EXISTS `data_history` (
  ...
  `completion_date_prev` date DEFAULT NULL,
  `completion_date_lastchanged` datetime DEFAULT NULL,
  `primary_completion_date_prev` date DEFAULT NULL,
  `primary_completion_date_lastchanged` datetime DEFAULT NULL,
...
CREATE PROCEDURE `generatePubmedNews`( IN days int)
BEGIN	
	DECLARE rtag_id INT;
	DECLARE frml VARCHAR(150);
	DECLARE score INT;
	...
	dynamicCursorLoop: LOOP
		...
		IF (stmt LIKE 'select %') THEN
			...
			SET @tmp_tbl = CONCAT('create temporary table lttmp2.t as ',REPLACE(stmt,"%d",days));
			PREPARE tmp_stmt2 FROM @tmp_tbl;
			EXECUTE tmp_stmt2;
