...
*/
function tindex_old($sourceid,$cat,$productz=NULL,$up_id=NULL,$cid=NULL,$productID=NULL)
{
	if($cat=='products') 
	{
...
//						$ln=strlen('  `source_id` = "'. $sourceid . '"  and  ');
						...
						{
							$new_lid=get_larvolid_old($sourceid);
...
//				$pos2 = stripos($query, $findme, $pos + strlen($findme));
				...
				foreach($nctidz as $key => $value)
				{
					...
					{
						$srch = array_search_partial_old($sourceid, $value); 
					...
					else
					{
						...
						if(trial_indexed_old($larvol_id,$cat,$cid)) // check if the trial+product/trial+area index already exists
						{
...
*/
function trial_indexed_old($larvol_id,$cat,$cid)
{
	$indextable=$cat=='products' ? 'product_trials' : 'area_trials';
...
function array_search_partial_old($string, $array)
{
...
function get_larvolid_old($osid)
{
$osid=trim($osid);
$query = "
				SELECT `larvol_id`
