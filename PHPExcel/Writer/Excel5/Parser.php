<?php
if(!defined('HACK_STRLEN')) define('HACK_STRLEN',2048);
...
class PHPExcel_Writer_Excel5_Parser
{
	...
	{
		if (preg_match("/^\"[^\"]{0," . HACK_STRLEN . "}\"$/", $token)) {
	...
	function _match($token)
	{
		switch($token) {
			...
			default:
				...
				elseif (preg_match("/^\"[^\"]{0," . HACK_STRLEN . "}\"$/",$token))
				{
					return $token;
				}
	...
	function _expression()
	{
		...
		if (preg_match("/^\"[^\"]{0," . HACK_STRLEN . "}\"$/", $this->_current_token)) {
