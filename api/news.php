...
function runNewsQuery($query) {
	...
	global $days;
	if( !empty($days) and !isset($_REQUEST['paged_mode']) ) 
	{
		$json = str_replace('"[', '[', $json);
		$json = str_replace(']"', ']', $json);
	}
...
if( isset($_REQUEST['paged_mode']) and $_REQUEST['paged_mode']=='yes')
{
	if((!$_REQUEST['page']>=1) or empty($_REQUEST['page']) )
		return;
	
	$newsids=getNewsCount($days) ;
	if(empty($newsids) or count($newsids)<1)
		return;
	$totalpages=ceil((count($newsids)/100));
	
	if($_REQUEST['page']>1)
	{
		$start=($_REQUEST['page']-1)*100;
		$end=$_REQUEST['page']*100;
	}
	else
	{
		$start=0;
	...
	if(empty($newsids[$start]))
		return;
	
	$newsdata = '[';
	$newsdata .= '{"total_pages":"'.$totalpages.'"}';
	for($i=$start; $i<$end; $i++)
	{
		if(empty($newsids[$i]))
			 break;
		$x=getIndividualNewsItem($newsids[$i]);
		$newsdata .=  ','.$x;
	}
	$newsdata .=  ']';
	echo $newsdata ;
...
function getIndividualNewsItem($id) {
	$query ='SELECT 
					CONCAT	(
								"[",GROUP_CONCAT(DISTINCT concat("{\"LI_id\":\"",p.LI_id),concat("\",\"name\":\"",REPLACE(p.name,\'"\',\'&quot;\')),concat("\",\"owner_sponsored\":\"",IF(pt.relation_type = \'ownersponsored\', 1, 0),"\"}")),"]"
							) 	
