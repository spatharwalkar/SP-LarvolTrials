...
foreach($rows as $row => $rval)
{
	...
	if(isset($productIds[$row]) && $productIds[$row] != NULL && !empty($areaIds))
	{
		...
		$htmlContent .= '<a id="Cell_Link_'.$online_HMCounter.'" href="'. trim(urlPath()) .'intermediary.php?p=' . $productIds[$row] . '&a=' . implode(',', $areaIds). '&list=1&sr=now&er=1 month&hm=' . $id . '" target="_blank" class="ottlink" style="text-decoration:underline; color:#000000;">'.$rval.$rowsCompanyName[$row].((trim($rowsTagName[$row]) != '') ? ' <b><font style="color:#0000A0; font-weight:bold">'.$rowsTagName[$row].'</font></b>':'').'</a>';
	}
	$htmlContent .= '</div></th>';
